...
查询成功的 ID 优先于选项或环境变量 `spanAttributes` 中的 `user.id`；查询及重试最终失败时保留静态配置，没有静态配置则写入 `user.id=unknown`，表示未取得有效身份。因开关关闭、地址无效或 token 为空而未发起查询时，保留静态配置，没有静态配置则省略。六类 span 从创建起统一使用这份配置快照，正文采集关闭时仍生效。修改 token 或环境变量需要重新初始化插件才会生效。
