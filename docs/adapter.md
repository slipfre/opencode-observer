...
内部关联标识在 provider 执行前移除，不通过 Observer 接口提交，也不写入 span。SDK 回调在进程内统一注册，但每个插件实例只处理与自身绑定的调用。实例释放时，移除该实例的监听器、清理待关联记录，并使旧绑定失效。共享回调中的失败隔离要求见 [总体架构 §6.1](architecture.md#61-执行与失败隔离)。
...
插件实例关闭时，释放适配层保存的事件处理状态、监听器和请求绑定。适配层与遥测层的关闭顺序、重复关闭的处理要求，以及数据导出职责，见 [总体架构 §6.2](architecture.md#62-生命周期契约)。
